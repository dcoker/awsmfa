...
restricted to harmless operations. Only in the presence of temporary credentials that are
acquired by this tool and flagged by IAM as having recently passed an MFA test will the full
policies be available to them.
...
awsmfa.py is intended to be used by organizations that use MFA with STS AssumeRole and IAM
...
Requirements
============
- The host running this script must have boto3 and python3 installed.
...
def main(argv):
    ...
    parser.add_argument('role_to_assume', nargs='?', metavar='role-to-assume',
                        help='Full ARN of the role you wish to assume. If not provided, '
                             'the temporary credentials will have the same permissions as the '
                             'user does but will also satisfy the aws:MultiFactorAuthPresent '
    ...
    parser.add_argument('--duration', type=int, default=SIX_HOURS_IN_SECONDS,
                        help='The number of seconds that you wish the temporary credentials to be '
                             'valid for.')
    parser.add_argument('--identity-profile', default='identity',
                        help='Name of the section in the credentials file representing your '
                             ...
                             'and session key replaced by the temporary credentials.')
