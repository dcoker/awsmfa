...
def main(args=None):
    ...
    try:
        ...
        print("Available profiles: %s" % ", ".join(sorted(session.available_profiles)))
        return USER_RECOVERABLE_ERROR

    serial_number = find_mfa_for_user(args.serial_number, session, session3)
    if not serial_number:
        print("There are no MFA devices associated with this user.", file=sys.stderr)
        return USER_RECOVERABLE_ERROR
...
def find_mfa_for_user(user_specified_serial, botocore_session, boto3_session):
    ...
    if user['User']['Arn'].endswith(':root'):
        # The root user MFA device is not in the same way as non-root users, so we must find the root MFA devices
        # using a different method than we do for normal users.
        devices = boto3_session.resource('iam').CurrentUser().mfa_devices.all()
        serials = (x.serial_number for x in devices)
    else:
        # Non-root users can have a restrictive policy that allows them only to list
        # devices associated with their user but it requires using the low level IAM
        # client to compose the proper request.
        username = user['User']['UserName']
        devices = botocore_session.create_client('iam').list_mfa_devices(UserName=username)
    ...
    if not serials:
        return None
    if len(serials) > 1:
        print("Warning: user has %d MFA devices. Using the first." % len(devices), file=sys.stderr)
    return serials[0]
